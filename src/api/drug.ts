...
export const getDrugs = async (filterParams?: DrugFilterParams) => {
  try {
    ...
    if (filterParams?.sortBy) {
      ...
      apiParams.ascending =
        filterParams.ascending !== undefined ? filterParams.ascending : true;
    }
...
export const getDrugsByDrugGroupId = async (drugGroupId: string) => {
  try {
    console.log(
      `Calling API: GET /drug-management/drugs/by-group/${drugGroupId}`
    );
    const response = await api.get(
      `/drug-management/drugs/by-group/${drugGroupId}`
    );
...
export const exportDrugsToExcel = async (
  ...
  try {
    ...
    const response = await api.post(
      "/drug-management/drugs/export-excel",
      exportRequest
    );
    ...
    try {
      ...
      const fullUrl = `https://api.truongvu.id.vn/api/drug-management/drugs/export-excel`;
...
export const getAvailableDrugsForPrescription = async () => {
  try {
    const response = await api.get(
      "/drug-management/drugs/available-for-prescription"
    );
    return response.data;
  } catch (error) {
