...
import { Typography } from "antd";
  ...
  export const setupStudentAppointmentRealTime = (
    ...
    connection.on("ReceiveSlotLocked", (data: { appointmentId: string; status: string }) => {
      console.log("Student: Slot locked", data);
      onUpdate({ appointmentId: data.appointmentId, status: data.status, eventType: "SlotLocked" });
    });
  
    connection.on("ReceiveAppointmentConfirmed", (data: { appointmentId: string; status: string }) => {
      console.log("Student: Appointment confirmed", data);
      onUpdate({ appointmentId: data.appointmentId, status: data.status, eventType: "Confirmed" });
    });
  
    connection.on("ReceiveSlotReleased", (data: { appointmentId: string; status: string }) => {
      console.log("Student: Slot released", data);
      onUpdate({ appointmentId: data.appointmentId, status: data.status, eventType: "Released" });
    });
    ...
    connection.onreconnected(() => {
      ...
      connection.invoke("SubscribeToUserUpdates", userId).catch((err) =>
