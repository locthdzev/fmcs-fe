...
export function InventoryRecordStatistic() {
  ...
  const exportToExcel = async (startDate?: Date, endDate?: Date) => {
    try {
      ...
      const exportInfo = [
        ...
        { info: 'Total Inventory Records', value: dataToExport.totalInventoryRecords || 0 },
      ...
      // Add summary data
      const summaryData = [
        { metric: 'Total Inventory Records', value: dataToExport.totalInventoryRecords || 0 },
        { metric: 'Total Active Inventory Records', value: dataToExport.totalActiveInventoryRecords || 0 },
        { metric: 'Total Inactive Inventory Records', value: dataToExport.totalInactiveInventoryRecords || 0 },
        { metric: 'Total Expired Inventory Records', value: dataToExport.totalExpiredInventoryRecords || 0 },
        { metric: 'Total Near Expiry Inventory Records', value: dataToExport.totalNearExpiryInventoryRecords || 0 },
        { metric: 'Total Priority Inventory Records', value: dataToExport.totalPriorityInventoryRecords || 0 },
        ...
        { metric: 'Active Quantity in Stock', value: dataToExport.activeQuantityInStock || 0 },
        { metric: 'Near Expiry Quantity in Stock', value: dataToExport.nearExpiryQuantityInStock || 0 },
        { metric: 'Priority Quantity in Stock', value: dataToExport.priorityQuantityInStock || 0 },
        { metric: 'Low Stock Items', value: dataToExport.lowStockItems || 0 },
        { metric: 'Zero Stock Items', value: dataToExport.zeroStockItems || 0 },
      ];
      ...
      const totalRecords = dataToExport.totalInventoryRecords || 1; // Prevent division by zero
      ...
      const lowStockSheet = workbook.addWorksheet('Low Stock Drugs');
      ...
      // Format header
      formatSheetHeader(lowStockSheet);
      
      // Add low stock drugs data
      dataToExport.drugsLowOnStock.forEach(drug => {
        const percentage = ((drug.totalQuantity) / (drug.reorderLevel || 1) * 100).toFixed(2) + '%';
        const row = lowStockSheet.addRow({ 
          drugName: drug.drugName, 
          drugCode: drug.drugCode, 
          quantity: drug.totalQuantity,
          reorderLevel: drug.reorderLevel || 'N/A',
          percentage
        });
        row.height = ROW_HEIGHT;
        row.eachCell({ includeEmpty: true }, (cell) => {
      ...
      >
            ...
            <Alert
              ...
              description={
                <div>
                  ...
                  top drugs by quantity, and low stock drugs.
                </div>
              }
              type="info"
              showIcon
        ...
        <div style={{ marginTop: "16px" }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: "8px" }}>
            ...
            <Tag color="red">Low Stock Drugs</Tag>
          </div>
        </div>
      </Modal>
    );
  ...
  const prepareChartData = () => {
    ...
    // Status quantity distribution data
    const statusQuantityData = [
      { name: "Active", value: statistics.activeQuantityInStock, fill: "#52c41a" },
      { name: "Near Expiry", value: statistics.nearExpiryQuantityInStock, fill: "#eb2f96" },
      { name: "Priority", value: statistics.priorityQuantityInStock, fill: "#faad14" },
    ];
  ...
  // Extract chart data
  const { statusChartData, drugsChartData, statusQuantityData } = prepareChartData();
  ...
  const renderLowStockTable = () => {
    const columns = [
      ...
      },
      {
        title: "Status",
        key: "status",
        render: (_: any, record: DrugStatisticsDTO) => (
          <Progress
            percent={(record.totalQuantity / (record.reorderLevel || 1)) * 100}
            size="small"
            status="exception"
            format={() => `${Math.round((record.totalQuantity / (record.reorderLevel || 1)) * 100)}%`}
          />
        ),
      },
    ];
    ...
    return (
      <div style={{ width: "100%" }}>
        <Alert
          ...
          description={`${statistics.lowStockItems} drugs are below their reorder level and require attention.`}
          type="warning"
          showIcon
        ...
        <div className="table-responsive">
          <div style={{padding: "16px", background: "#fff", borderRadius: "8px"}}>
            {statistics.drugsLowOnStock.length > 0 ? (
              <div style={{ overflow: 'auto' }}>
                <table className="ant-table" style={{ width: "100%" }}>
                  ...
                  <tbody className="ant-table-tbody">
                    {statistics.drugsLowOnStock.map((drug, index) => (
                      <tr key={index} className="ant-table-row">
                        <td><Tag color="blue">{drug.drugCode}</Tag></td>
                        <td><strong>{drug.drugName}</strong></td>
                        <td><Tag color="red"><strong>{drug.totalQuantity}</strong></Tag></td>
                        <td><Tag color="orange"><strong>{drug.reorderLevel}</strong></Tag></td>
                        <td>
                          <Progress
                            percent={(drug.totalQuantity / (drug.reorderLevel || 1)) * 100}
                            size="small"
                            status="exception"
                            format={() => `${Math.round((drug.totalQuantity / (drug.reorderLevel || 1)) * 100)}%`}
                          />
                        </td>
                      </tr>
                    ))}
              ...
              <Empty description="No drugs are below reorder level" />
            )}
          </div>
        </div>
      </div>
  ...
  // Render status quantity distribution chart
  const renderStatusQuantityChart = () => {
    const { statusQuantityData } = prepareChartData();

    return (
      <div style={{ width: "100%" }}>
        <div className="mb-4">
          <Alert
            message="Quantity by Status"
            description="Distribution of drug quantities across different status categories"
            type="info"
            showIcon
          />
        </div>
        <Row gutter={16}>
          <Col span={12}>
            <ResponsiveContainer width="100%" height={400}>
              <PieChart>
                ...
                >
                  {statusQuantityData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.fill || COLORS[index % COLORS.length]}
                    />
          ...
          <Col span={12}>
            <Card title="Quantity Distribution by Status" bordered={false}>
              <div style={{ marginBottom: "16px" }}>
                <Title level={5}>Active: {statistics.activeQuantityInStock}</Title>
                <Progress 
                  percent={(statistics.activeQuantityInStock / statistics.totalQuantityInStock) * 100} 
                  strokeColor="#52c41a" 
                />
              </div>
              <div style={{ marginBottom: "16px" }}>
                <Title level={5}>Near Expiry: {statistics.nearExpiryQuantityInStock}</Title>
                <Progress 
                  percent={(statistics.nearExpiryQuantityInStock / statistics.totalQuantityInStock) * 100} 
                  strokeColor="#eb2f96" 
                />
              </div>
              <div style={{ marginBottom: "16px" }}>
                <Title level={5}>Priority: {statistics.priorityQuantityInStock}</Title>
                <Progress 
                  percent={(statistics.priorityQuantityInStock / statistics.totalQuantityInStock) * 100} 
                  strokeColor="#faad14" 
                />
              </div>
              <div>
                <Title level={5}>Total: {statistics.totalQuantityInStock}</Title>
                <Progress percent={100} strokeColor="#1677ff" />
              </div>
            </Card>
          </Col>
        </Row>
  ...
  const tabItems = [
    ...
    },
    {
      ...
      label: (
        <Space align="center">
          Low Stock Drugs
          <AntTooltip title="Shows drugs that are below their reorder level">
            <QuestionCircleOutlined />
          </AntTooltip>
        </Space>
      ),
      children: renderLowStockTable(),
    },
    {
      key: "4",
      label: (
        <Space align="center">
          Status Quantity Distribution
          <AntTooltip title="Shows the quantity of drugs by their status">
            <QuestionCircleOutlined />
          </AntTooltip>
        </Space>
      ),
      children: renderStatusQuantityChart(),
    },
  ];
      ...
      <Row gutter={16} className="mb-6">
            ...
            <Statistic
              title={
                <Space align="center">
                  ...
                  <AntTooltip title="Total number of inventory records">
                    <QuestionCircleOutlined
                      style={{
                        fontSize: "14px",
              ...
              value={statistics.totalInventoryRecords}
        ...
        <Col xs={24} md={8}>
          ...
          >
            <Statistic
              title={
                <Space align="center">
                  ...
                  <AntTooltip title="Number of active inventory records">
                    <QuestionCircleOutlined
                      style={{
                        fontSize: "14px",
                        color: token.colorTextSecondary,
            ...
            <div style={{ marginTop: "10px" }}>
              <Progress
                percent={(statistics.totalActiveInventoryRecords / statistics.totalInventoryRecords) * 100}
                showInfo={false}
                strokeColor="#52c41a"
              />
            </div>
      ...
      <Row gutter={16} className="mb-6">
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            ...
            <div style={{ marginTop: "10px" }}>
              <Progress
                percent={(statistics.lowStockItems / statistics.totalInventoryRecords) * 100}
                showInfo={false}
                strokeColor="#faad14"
              />
            </div>
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <Statistic
              title={
                <Space align="center">
                  ...
                  <AntTooltip title="Number of items with zero quantity">
                    <QuestionCircleOutlined
                      style={{
                        fontSize: "14px",
                        color: token.colorTextSecondary,
            ...
            <div style={{ marginTop: "10px" }}>
              <Progress
                percent={(statistics.zeroStockItems / statistics.totalInventoryRecords) * 100}
                showInfo={false}
                strokeColor="#f5222d"
              />
            </div>
            ...
            <Statistic
              title={
                <Space align="center">
                  ...
                  <AntTooltip title="Number of near expiry inventory records">
                    <QuestionCircleOutlined
                      style={{
                        fontSize: "14px",
                        color: token.colorTextSecondary,
              ...
              value={statistics.totalNearExpiryInventoryRecords}
              prefix={<WarningOutlined style={{ color: "#eb2f96" }} />}
              valueStyle={{ color: "#eb2f96" }}
            />
            <div style={{ marginTop: "10px" }}>
              <Progress
                percent={(statistics.totalNearExpiryInventoryRecords / statistics.totalInventoryRecords) * 100}
                showInfo={false}
                strokeColor="#eb2f96"
              />
            </div>
