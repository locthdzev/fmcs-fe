...
import { exportHealthCheckStatisticsToExcel } from "./export/health-check-statitics";
...
export function HealthCheckResultStatistics() {
  ...
  // Export modal states
  const [exportDateRange, setExportDateRange] = useState<[Date | undefined, Date | undefined]>([
    undefined,
    undefined,
  ]);
  ...
  const exportToExcel = async () => {
    ...
    try {
      // Sử dụng date range từ modal nếu có, ngược lại dùng date range chính
      ...
      const response = {
        ...
        data: statistics,
        responseFailed: null,
        message: "Health check results statistics exported successfully"
      };
      
      console.log("Exporting with date range:", { startDate, endDate });
      
      // Gọi hàm export với response và date range
      exportHealthCheckStatisticsToExcel(
        response,
        "Health Check Results Statistics Report",
        startDate,
        endDate
      );
