...
export const Survey: React.FC<SurveyProps> = ({ id, onSuccess, readOnly = false }) => {
  ...
  // Always set readOnly to true for Healthcare Staff
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="bg-red-50 p-6 text-center">
          ...
          <Button 
            type="primary" 
            size="large" 
            onClick={() => router.push('/survey/StaffSurvey')}
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden card-clean">
          ...
          <div className="flex justify-between items-center">
            <div>
              <Title level={2} style={{ color: 'white', margin: 0 }}>
                {getStatusEmoji(survey.status)} Patient Feedback
              </Title>
        ...
        <div className="p-6">
          ...
          <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* User info for healthcare staff view */}
            <Card 
            ...
            >
              <div className="flex items-start">
                ...
                <div>
                  <Text type="secondary" className="block mb-1">Patient</Text>
                  <Text strong className="text-lg block">{survey.user?.fullName}</Text>
                  <Text type="secondary">{survey.user?.email}</Text>
                </div>
              </div>
            </Card>
          ...
          {/* Rating form - always in read-only mode for Healthcare Staff */}
          <Card
          ...
          <div className="text-center text-gray-500 text-sm mb-4">
            <p>This is a read-only view of the patient's feedback.</p>
            <p>You cannot modify this survey.</p>
          </div>
...
// Component to display staff's survey list
export const SurveyList: React.FC<SurveyListProps> = ({ userId, onSelectSurvey }) => {
  ...
  const fetchStaffSurveys = async (userIdToUse: string | undefined) => {
    if (!userIdToUse) {
    ...
    try {
      ...
      // For healthcare staff, fetch surveys related to them
      const response = await getSurveysByStaffId(userIdToUse, {
        page: 1,
        pageSize: 50,
      });
      console.log('Healthcare Staff Survey API Response:', JSON.stringify(response));
      ...
      if (response && response.isSuccess) {
        ...
        if (totalItems === 0) {
          setError('You don\'t have any surveys related to you yet.');
        }
        ...
        setError('Unable to load surveys related to you.');
      }
    } catch (error: any) {
      console.error('Error in fetchStaffSurveys:', error);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-80 bg-gray-50 rounded-lg shadow-sm border border-gray-200">
        <Spin size="large" />
        <p className="mt-6 text-gray-500 animate-pulse">Loading surveys...</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-8 text-center">
          ...
          <Paragraph className="text-gray-500 mb-6">
            You currently don't have any feedback related to your services.
            Surveys will appear here after patients complete them.
          </Paragraph>
          <Button 
            type="primary" 
            size="large"
      ...
      <Card 
        title={
          <div className="flex items-center">
            ...
            <span className="text-lg font-semibold">Staff's Surveys</span>
          </div>
        }
        className="shadow-md"
      >
        ...
        <div className="bg-gradient-to-r from-teal-500 to-blue-600 rounded-lg shadow-md p-6 text-white">
          <div className="mb-4">
            <Title level={2} style={{ color: 'white', margin: 0 }}>
              Surveys For My Services
            </Title>
          </div>
          <Paragraph style={{ color: 'rgba(255, 255, 255, 0.85)', marginBottom: 20 }}>
            View patient feedback for the services you've provided
          </Paragraph>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
            {filteredSurveys.map((survey, index) => (
              ...
              >
                ...
                <div className="p-5">
                  {/* Show user info instead of staff info for Healthcare Staff */}
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center">
                      <Avatar icon={<UserOutlined />} className="mr-2" />
                      <div>
                        <div className="font-medium">{survey.user?.fullName}</div>
                        <div className="text-xs text-gray-500">{survey.user?.email}</div>
                      </div>
                    </div>
                  ...
                  {survey.rating > 0 && (
                    <div className="mb-4">
                      <div className="text-sm text-gray-500 mb-1">Patient Rating</div>
                      <div className="flex items-center">
