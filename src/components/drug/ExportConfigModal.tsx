  ...
  const handleSubmit = async () => {
    try {
      ...
      const useFilters = {
        ...
        drugCodeSearch: values.exportAllPages
          ? undefined
          : values.filterDrugCode,
        nameSearch: values.exportAllPages ? undefined : values.filterName,
        manufacturerSearch: values.exportAllPages
          ? undefined
          : values.filterManufacturer,
        descriptionSearch: values.exportAllPages
          ? undefined
          : values.filterDescription,
        ...
        status: values.exportAllPages
          ? undefined
          : values.filterStatus?.length > 0
          ? values.filterStatus?.join(",")
          : undefined,
        // Don't apply date filters when exporting all data
        createdStartDate: values.exportAllPages
          ? undefined
          : values.filterCreatedDateRange?.[0]?.format("YYYY-MM-DD") ||
            undefined,
        createdEndDate: values.exportAllPages
          ? undefined
          : values.filterCreatedDateRange?.[1]?.format("YYYY-MM-DD") ||
            undefined,
        updatedStartDate: values.exportAllPages
          ? undefined
          : values.filterUpdatedDateRange?.[0]?.format("YYYY-MM-DD") ||
            undefined,
        updatedEndDate: values.exportAllPages
          ? undefined
          : values.filterUpdatedDateRange?.[1]?.format("YYYY-MM-DD") ||
            undefined,
      };
                    ...
                    <Col span={12}>
                      <Form.Item label="Manufacturer" name="filterManufacturer">
                        <Select
                          ...
                          options={Array.from(
                            new Set(
                              drugs.map((d) => d.manufacturer).filter(Boolean)
                            )
                          ).map((manufacturer) => ({
                            value: manufacturer,
                            label: manufacturer,
                    ...
                    <Col span={12}>
                      <Form.Item label="Description" name="filterDescription">
                        <Select
                          ...
                          options={Array.from(
                            new Set(
                              drugs.map((d) => d.description).filter(Boolean)
                            )
                          ).map((description) => ({
                            value: description,
                            label: description,
