...
const ExportConfigModal: React.FC<ExportConfigModalProps> = ({
  ...
  // Reset form when modal becomes visible or config changes
  useEffect(() => {
    if (visible) {
      handleReset();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [visible]); // Chỉ chạy khi visible thay đổi, không phụ thuộc vào config
  ...
  const handleReset = () => {
    ...
    const defaultValues = {
      ...
      filterSortDirection: filters.advancedFilters?.ascending ? "asc" : "desc",
    ...
    // Chỉ cập nhật form, không gọi onChange nơi đây
    form.setFieldsValue(defaultValues);
    
    // Tách biệt xử lý config và gọi onChange riêng
    const configOnly = {
      exportAllPages: true,
      includeSupplierName: true,
      includeContactNumber: true,
    ...
    // Chỉ gọi onChange một lần với các trường config
    onChange(configOnly);
  };
      ...
      <Spin spinning={loading} tip="Generating Excel file..."> 
        <Form
          ...
          onValuesChange={(changedValues) => {
            // Thêm logic để chọn tất cả Include Fields khi Export all data được chọn
            ...
            if (Object.keys(changedValues).some(key => 
              ...
              key === 'exportAllPages')) {
              // Chỉ gọi onChange cho config (không gọi cho tất cả giá trị)
        ...
        >
          <div style={{ marginBottom: '20px' }}>
            <h3 style={{ fontSize: '16px', marginBottom: '16px' }}>Basic Options</h3>
          ...
          <Form.Item dependencies={["exportAllPages"]} noStyle>
            {({ getFieldValue }) => {
              ...
              return !exportAll ? (
                <div style={{ marginBottom: '20px' }}>
                  <h3 style={{ fontSize: '16px', marginBottom: '16px' }}>Data Filters</h3>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h3 style={{ fontSize: '16px', marginBottom: '16px' }}>Include Fields</h3>
