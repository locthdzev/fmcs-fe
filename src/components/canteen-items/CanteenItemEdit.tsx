...
export const CanteenItemEdit: React.FC<CanteenItemEditProps> = ({ id }) => {
  ...
  const [canteenItem, setCanteenItem] = useState<CanteenItemResponse | null>(
    null
  );
  ...
  const fetchCanteenItemDetails = async () => {
    ...
    try {
      ...
      if (response && typeof response === "object") {
        ...
        messageApi.error(
          "Failed to fetch canteen item details: Invalid data format"
        );
        setCanteenItem(null);
      }
    } catch (error) {
      ...
      if (fileList.length > 0) {
        ...
        // Trường hợp 2: File không có originFileObj nhưng có url
          ...
          try {
            ...
            imageFile = new File([blob], fileList[0].name || "image.jpg", {
              type: blob.type,
            });
        ...
        // Trường hợp 3: Có file trong fileList nhưng không thể xử lý
          ...
          messageApi.success(
            "Canteen item updated successfully (without new image)"
          );
          ...
          >
            ...
            >
              ...
              <div className="flex justify-end gap-2">
                <Button onClick={() => router.push(`/canteen-item/${id}`)}>
                  Cancel
                </Button>
                <Button
                  type="primary"
