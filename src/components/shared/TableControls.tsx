...
const TableControls: React.FC<TableControlsProps> = ({
  ...
  const filteredPageSizeOptions = pageSizeOptions.filter(size => size <= maxRowsPerPage);
            ...
            {bulkActions
              ...
              .map(action => (
                <Tooltip key={action.key} title={action.tooltip}>
                  <Popconfirm
                    title={<div style={{ padding: "0 10px" }}>{action.title}</div>}
                    description={<p style={{ padding: "10px 40px 10px 18px" }}>{action.description}</p>}
