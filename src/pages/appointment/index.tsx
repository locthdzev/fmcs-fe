...
const AppointmentIndexPage: React.FC<AppointmentIndexPageProps> = ({ initialStaffList, token }) => {
  const [staffList, setStaffList] = useState<AvailableOfficersResponseDTO[]>(initialStaffList);
  ...
  useEffect(() => {
    ...
    const cleanupSignalR = setupHealthcareStaffRealTime((updatedStaffList: AvailableOfficersResponseDTO[]) => {
            ...
            <div className="flex flex-wrap text-black">
              <div className="pr-10">
                <div className="text-2xl font-bold">
                  {statistics ? statistics.totalHealthcareOfficers : "Loading..."}
                </div>
                <div>Total Healthcare Officers</div>
              </div>
              <div className="pr-10">
                <div className="text-2xl font-bold">
                  {statistics ? statistics.studentsCurrentlyReceivingCare : "Loading..."}
                </div>
                <div>Students Currently Receiving Care</div>
              </div>
              <div className="pr-10">
                <div className="text-2xl font-bold">
                  {statistics ? statistics.appointmentsScheduledToday : "Loading..."}
                </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-wrap w-full">
              {staffList.map((staff, index) => {
                const colors = [
                  { bg: "bg-blue-100", text: "text-blue-700", bar: "bg-blue-700" },
                  { bg: "bg-yellow-100", text: "text-yellow-700", bar: "bg-yellow-700" },
                  { bg: "bg-green-100", text: "text-green-700", bar: "bg-green-700" },
                  { bg: "bg-purple-100", text: "text-purple-700", bar: "bg-purple-700" },
                  { bg: "bg-red-100", text: "text-red-700", bar: "bg-red-700" },
                  { bg: "bg-cyan-100", text: "text-cyan-700", bar: "bg-cyan-700" },
                ];
                    ...
                    <Link href={`/appointment/schedule/${staff.staffId}`}>
                      ...
                      >
                        ...
                        <div className="mt-1 text-center">
                          <p className="text-base font-bold">{staff.fullName}</p>
                          <p className="mt-1 text-sm opacity-70">General Physician</p>
                        </div>
                      </div>
                    </Link>
                  </div>
...
const getAllHealthcareStaffWithToken = async (token: string): Promise<ResultDTO<AvailableOfficersResponseDTO[]>> => {
  const axios = (await import("axios")).default;
  const https = (await import("https")).default;

  const response = await axios.get("http://localhost:5104/api/appointment-management/healthcare-staff", {
    headers: {
  ...
  if (!response.data.isSuccess) {
    throw new Error(response.data.message || "Failed to fetch healthcare staff");
  }
