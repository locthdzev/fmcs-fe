...
const ScheduleAppointment: React.FC<ScheduleAppointmentProps> = ({ staff, token }) => {
  ...
  const [conflictAppointment, setConflictAppointment] = useState<AppointmentResponseDTO | null>(null);
  ...
  const fetchAvailableSlots = useCallback(
    debounce(async (date: string) => {
      ...
      try {
        ...
        const userLockedSlot = slotData.find((slot) => slot.lockedByCurrentUser);
        if (userLockedSlot) {
          ...
          selectedTimeSlot &&
          !slotData.some((s) => s.timeSlot === selectedTimeSlot && (s.isAvailable || s.lockedByCurrentUser))
        ) {
          resetSelectionState();
        }
      } catch (error) {
  ...
  const updateSlotState = useCallback((timeSlot: string, updates: Partial<TimeSlot>) => {
    setAvailableSlots((prev) => {
      const currentSlots = Array.isArray(prev) && prev.length > 0 
        ? prev 
        : allTimeSlots.map((ts) => ({
            timeSlot: ts,
            isAvailable: true,
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    broadcastChannelRef.current.onmessage = (event) => {
      const { type, appointmentId: broadcastedId, lockedUntil, timeSlot, selectedDate: broadcastedDate, staffId, canceledTimeSlot, reason } = event.data;
      console.log(`BroadcastChannel message received - User: ${userId}`, event.data);

      if (type === "LOCK_SLOT" && broadcastedDate === selectedDateRef.current && staffId === staff.staffId) {
        updateSlotState(timeSlot, { isAvailable: false, isLocked: true, lockedByCurrentUser: true });
      ...
      } else if (type === "SLOT_LOCKED_BY_OTHER" && broadcastedDate === selectedDateRef.current && staffId === staff.staffId) {
        updateSlotState(timeSlot, { isAvailable: false, isLocked: true, lockedByCurrentUser: false });
        // toast.info(`Slot ${timeSlot} is now taken`);
      } else if (type === "CANCEL_SLOT" && broadcastedDate === selectedDateRef.current) {
        updateSlotState(canceledTimeSlot, { isAvailable: true, isLocked: false, lockedByCurrentUser: false });
      ...
      } else if (type === "CONFIRM_APPOINTMENT" && broadcastedId === appointmentIdRef.current && staffId === staff.staffId) {
        updateSlotState(timeSlot, { isAvailable: false, isLocked: true, lockedByCurrentUser: false, isConfirmed: true });
        resetSelectionState();
        setIsConfirmed(true);
        setLoading(false);
        setIsConfirming(false);
        toast.success("Appointment confirmed in another tab!");
      } else if (type === "CONFIRM_START" && broadcastedId === appointmentIdRef.current && staffId === staff.staffId) {
        setLoading(true);
        setIsConfirming(true);
        toast.info("Confirmation started in another tab");
      } else if (type === "CONFIRM_END" && broadcastedId === appointmentIdRef.current && staffId === staff.staffId) {
        setLoading(false);
        setIsConfirming(false);
      }
    };
  ...
  useEffect(() => {
    ...
    console.log(`SignalR setup initiated with User: ${userId}, Staff: ${staff.staffId}, Token: ${token.substring(0, 10)}...`);
    const signalRManager = SignalRManager.getInstance();
    const group = `Staff_${staff.staffId}`;
  
    let unsubscribeGroup: () => Promise<void> = async () => {};
    signalRManager.subscribeToGroup(group).then(cleanup => {
      unsubscribeGroup = cleanup;
    }).catch(err => console.error("Failed to subscribe to group:", err));
  
    const cleanupScheduleLocked = signalRManager.on("ReceiveSlotLocked", (data) => {
      console.log(`SignalR ScheduleLocked Received - User: ${userId}`, data);
      if (selectedDateRef.current === data.date && !schedulingInProgressRef.current) {
        updateSlotState(data.timeSlot, { isAvailable: false, isLocked: true, lockedByCurrentUser: false });
        ...
        console.log(`Broadcast sent for SLOT_LOCKED_BY_OTHER: ${data.timeSlot}`);
      }
    });
  
    const cleanupCancelExpiredLocked = signalRManager.on("CancelExpiredLockedAppointment", (data) => {
      console.log(`SignalR CancelExpiredLocked Received - User: ${userId}`, data);
      // Always update slot state if date matches, regardless of current selection
      if (selectedDateRef.current === data.date) {
        updateSlotState(data.timeSlot, { isAvailable: true, isLocked: false, lockedByCurrentUser: false });
        broadcastChannelRef.current?.postMessage({
          type: "CANCEL_SLOT",
        ...
        console.log(`Broadcast Sent (Expired) - User: ${userId}, Slot: ${data.timeSlot}, Date: ${data.date}`);
        // Only reset selection if this user owns the slot
        if (data.timeSlot === selectedTimeSlotRef.current && data.appointmentId === appointmentIdRef.current) {
          resetSelectionState();
        ...
        updateSlotState(data.timeSlot, { isAvailable: true, isLocked: false, lockedByCurrentUser: false });
        console.log(`Slot ${data.timeSlot} released for date ${data.date}, no fetch triggered (date mismatch)`);
      }
    });
  ...
  const cleanupConfirmAppointment = signalRManager.on("ReceiveAppointmentConfirmed", (data) => {
    ...
    return () => {
      console.log(`Cleaning up SignalR for User: ${userId}, Staff: ${staff.staffId}`);
  ...
  useEffect(() => {
    ...
    while (daysAdded < 32) {
      if (currentDay.day() !== 0) { // Exclude Sundays
...
const useCountdown = (
  ...
  useEffect(() => {
    if (!lockedUntil || !appointmentId || !selectedTimeSlot || isConfirmed || isConfirming) {
      setTimeLeft(null);
      return;
    }
    ...
    const interval = setInterval(() => {
      const diff = moment(lockedUntil).diff(moment());
      console.log("Countdown tick", { diff, timeLeft, hasHandledExpiration: hasHandledExpirationRef.current });
  ...
  }, [lockedUntil, appointmentId, selectedTimeSlot, isConfirmed, isConfirming]);
...
const handleLockExpiration = useCallback(
  debounce(async () => {
    ...
    if (!token || !selectedTimeSlot || !selectedDate || !appointmentId || hasHandledExpirationRef.current) return;

    setLoading(true);
    try {
      const response = await cancelExpiredLockedAppointment(appointmentId, token);
      if (response.isSuccess) {
        updateSlotState(selectedTimeSlot, { isAvailable: true, isLocked: false, lockedByCurrentUser: false });
        broadcastChannelRef.current?.postMessage({
          type: "CANCEL_SLOT",
        ...
        toast.error(`Failed to release expired slot: ${response.message || "Unknown error"}`);
      }
    } catch (error) {
  ...
  [token, selectedTimeSlot, selectedDate, appointmentId, staff.staffId, fetchAvailableSlots]
);
...
const handleDateChange = (index: number) => {
  ...
  setIsConfirming(false); // Reset confirming state
    try {
      ...
      if (!validationResponse.isSuccess) {
        if (validationResponse.code === 409 && validationResponse.message?.includes("You already have a locked appointment")) {
          ...
          if (existingAppointmentId) {
            ...
            let currentStaffName = existingAppointment.staffName || "Unknown Staff";
        if (!existingAppointment.staffName && existingAppointment.staffId) {
          const staffResponse = await getHealthcareStaffById(existingAppointment.staffId, token);
          currentStaffName = staffResponse.data?.fullName || "Unknown Staff";
        }
      ...
      if (scheduleResponse.isSuccess && scheduleResponse.data) {
        ...
        setIntendedTimeSlot(null);
      } else if (scheduleResponse.code === 409 && scheduleResponse.message?.includes("You already have a locked appointment")) {
  ...
  const handleConflictConfirm = async () => {
    if (!token || !selectedDate || !intendedTimeSlot || !conflictAppointment) return;
    if (!selectedDate || !token || !userId) {
      toast.error("Missing required information for scheduling");
      return;
    }
    ...
    console.log("Attempting to cancel present locked appointment", { appointmentId: conflictAppointment.id });
    const cancelResponse = await cancelExpiredLockedAppointment(conflictAppointment.id, token);
    console.log("Cancel response received", { isSuccess: cancelResponse.isSuccess, message: cancelResponse.message });
    if (cancelResponse.isSuccess) {
      const canceledStartMoment = moment.tz(conflictAppointment.appointmentDate, "Asia/Ho_Chi_Minh");
      const canceledEndMoment = moment.tz(
        conflictAppointment.endTime || canceledStartMoment.clone().add(30, "minutes"),
        "Asia/Ho_Chi_Minh"
      );
      ...
      // Schedule the new appointment
        ...
        if (scheduleRetry.isSuccess && scheduleRetry.data) {
          ...
          updateSlotState(intendedTimeSlot, { isAvailable: false, isLocked: true, lockedByCurrentUser: true });
          broadcastChannelRef.current?.postMessage({
            type: "LOCK_SLOT",
  ...
  const handleConfirmAppointment = async () => {
    ...
    setIsConfirming(true);
    try {
      const response = await confirmAppointment(appointmentId, token, appointmentReason.trim());
      if (response.isSuccess) {
        console.log("Updating slot to confirmed state", { timeSlot: selectedTimeSlot });
  ...
  const renderTimeSlotButton = useCallback(
    (slot: TimeSlot, index: string) => {
      ...
      const buttonStyle = {
        ...
        backgroundColor: isLocked && !lockedByCurrentUser && !isConfirmed
       ...
       border: isSelected
        ...
        boxShadow: isHovered && !isDisabled ? "0 4px 8px rgba(0, 0, 0, 0.1)" : "none",
        ...
        <Col key={dateStr} xs={8} sm={6} md={4} lg={3}>
          ...
          >
            ...
            {fetchingSlotCounts ? (
              ...
              <span style={{ fontSize: "12px", color: "#28c41a", marginTop: "4px" }}>
                {slotCount} slots
              </span>
            ) : (
              <span style={{ fontSize: "12px", color: "#f5222d", marginTop: "4px" }}>
                Fully Booked
              </span>
            )}
          </Button>
